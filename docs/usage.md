...
| `pool`   | Pool name for the current sample. Samples sharing the same pool will be analysed together since CNV calling in the ExomeDepth workflow is executed per pool. It's recommended to put the (genetically speaking) female and male samples in seperate pools. The samples in the same pool should be enriched with the same library prep kit. An optimized aggregated reference set is chosen from the samples of the pool. Cannot contain spaces. |
| `family` | Family name for the current sample. Samples sharing this family name will be excluded from the aggregate reference set to ensure that common CNVs to this family will be detected. ExomeDepth assumes that the CNV of interest is absent from the aggregate reference set. Cannot contain spaces.                                                                                                                                               |
